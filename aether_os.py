...
from experiri.model_loader import load_player  # USE THE NEW SYSTEM
import aiohttp
...
class Contextus:
    ...
    async def execute_command(self, cmd):
        """Parses and executes a command, now handling async verbs."""
        try:
    ...
    def _get_verb_handlers(self):
        ...
        return {
            ...
            "AMOR": self._handle_amor,  # Added AMOR
        }
    ...
    def _handle_instauro(self, inf, mod, lit, args):
        name = lit[0].upper()
        arch = (re.search(r"MODO\s+'([^']*)'", args.upper()) or [None, "TRANSFORMER"])[
            1
        ]
        if name in self.materiae:
            return f"'{name}' IAM EXISTIT"
    ...
    def _handle_perturbo(self, inf, mod, lit, args):
        ...
        if lit:
            ...
            if raw_amp > max_amplitude:
                ...
                print(
                    f"< INFO: Input flux saturated. Amplitude capped at {max_amplitude:.2f}. Excess: {excess_energy:.2f} >"
                )
                log_event(
                    f"Perturbo overflow: Excess energy {excess_energy:.2f} for {self.focus}"
                )

                if excess_energy > 10:  # Adjusted threshold for testing
                    print(
                        "< CRITICAL FLUX OVERFLOW: Engaging Carlyle Stability Protocol. >"
                    )
                    ...
                    if discriminant >= 0:
                        ...
                        core.magnetism += (
                            excess_energy * 0.05
                        )  # Fallback for imaginary roots
                        log_event(
                            f"Fallback applied: Magnetism adjusted by {excess_energy * 0.05:.2f}"
                        )
        else:
            amp = 1.0
    ...
    async def _handle_interrogo(self, inf, mod, lit, args):
        core = self.get_focused_materia()
        # The syntax ORACULO 'model_key' is now used to specify a model from the registry
        model_key = (
        ...
        # Extract the text response, accommodating both Ollama and Gemini player formats
        if isinstance(response_data, dict) and "error" in response_data:
            response_text = f"ORACULUM ERRORUM: {response_data['error']}"
        elif isinstance(response_data, dict):
            # For Ollama, the main text is usually in a 'response' or similar key
            response_text = response_data.get("response", str(response_data))
        else:
            response_text = str(response_data)  # Fallback for other formats
    ...
    def _handle_doceo(self, inf, mod, lit, args):
        ...
        target_core, source_core = self.materiae.get(target_name), self.materiae.get(
            source_name
        )
        if not target_core:
...
async def main():
    """Main async function to run the AetherOS REPL."""
    context = Contextus()
    print("\n--- AetherOS v3.3 REPL ---")
    ...
    while True:
        try:
            ...
            if not cmd.strip():
                continue
            if cmd.lower() == "test":
                # Running tests remains synchronous for simplicity
                run_tests()
                continue
